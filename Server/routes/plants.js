...
router.get('/names', function(req, res, next) {
    ...
    db.query('SELECT plants.Id as Id, plants.Name as Name FROM plants', function(rows) {
        if (rows.localeCompare("[]")) {
            res.send(rows);
        } else {
...
router.get('/descriptions', function(req, res, next) {
    ...
    db.query('SELECT plants.Id as Id, plants.Description as Description FROM plants', function(rows) {
        if (rows.localeCompare("[]")) {
            res.send(rows);
        } else {
            res.send("No plants have been found");
...
router.get('/coordinates', function(req, res, next) {
    ...
    db.query('SELECT plants.Id as Id, plants.CoordinateX as CoordinateX, plants.CoordinateY as CoordinateY FROM plants', function(rows) {
        if (rows.localeCompare("[]")) {
            res.send(rows);
        } else {
            res.send("No plants have been found");
...
router.post('/upload', function(req, res) {
    ...
    if (!name) {
        ...
        db.query('INSERT INTO plants (Name, Description, CoordinateX, CoordinateY, PillarId) VALUES ("' + name + '", "' + description + '", ' + coordinateX + ', ' + coordinateY + ', ' + pillarId + ')', function () {
...
router.get('/update/:id', function(req, res, next) {
    ...
    if (isNaN(id)) {
        ...
        db.query('SELECT * FROM plants WHERE plants.Id = ' + id, function (plantsRows) {

            if (plantsRows.localeCompare("[]") === 0) {
                res.send("No plant with this Id have been found");
            } else {

                db.query('SELECT pillars.Id as PillarId FROM pillars', function(pillarsRows) {
...
router.post('/updateData', function(req, res) {
    ...
    if (!name) {
        ...
        db.query('UPDATE plants SET Name="' + name + '", Description="' + description + '", CoordinateX=' + coordinateX + ', CoordinateY=' + coordinateY + ', PillarId=' + pillarId + ' WHERE Id = ' + id, function () {
...
router.get('/:id', function(req, res, next) {
    ...
    if (isNaN(id)) {
        ...
        db.query('SELECT * FROM plants WHERE plants.Id = ' + id, function (rows) {
...
router.get('/:id/coordinates', function(req, res, next) {
    ...
    if (isNaN(id)) {
        ...
        db.query('SELECT plants.CoordinateX as CoordinateX, plants.CoordinateY as CoordinateY FROM plants WHERE plants.Id = ' + id, function (rows) {
...
router.get('/:id/images', function(req, res, next) {
   ...
   if (isNaN(id)) {
       ...
       db.query('SELECT * FROM images WHERE images.PlantId = ' + id, function (rows) {
